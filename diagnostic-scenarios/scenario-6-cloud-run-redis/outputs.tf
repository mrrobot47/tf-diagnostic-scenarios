...
output "verification_command_logs" {
  ...
  value       = "gcloud logging read \"resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"test-sc-6-redis-direct-connect\" AND severity>=DEFAULT\" --project=${var.project_id} --limit=20 --format='value(textPayload)'"
}
